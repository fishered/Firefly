(function initializeFireflyI18n() {
  const ENGLISH = {
    ...
    '任务管理': 'Jobs',
    '业务日历': 'Calendars',
    '让任务跟随业务时间运行': 'Run jobs on business time',
    '每套日历拥有独立时区、每周工作模板和日期例外。任务绑定后，调度器会按该日历判断是否创建执行。': 'Each calendar has its own time zone, weekly template, and date exceptions. Bound jobs use it to decide whether an execution should be created.',
    '创建业务日历': 'Create business calendar',
    '查看并编辑日历': 'View and edit calendar',
    '暂无未来日期例外': 'No upcoming date exceptions',
    '每周工作模板': 'Weekly work template',
    '作为所有日期的默认规则': 'Default rule for all dates',
    '日期例外': 'Date exceptions',
    '优先覆盖每周工作模板': 'Overrides the weekly template',
    '恢复默认': 'Restore default',
    '开始日期': 'Start date',
    '结束日期': 'End date',
    '应用到日期范围': 'Apply to date range',
    '将类型应用到已选日期': 'Apply type to selected dates',
    '清空当前选择': 'Clear selection',
    '已选': 'Selected',
    '当前类型：': 'Current type: ',
    '天': 'days',
    '点击可多选；按住 Shift 选择连续范围，拖拽也可以快速框选。': 'Click to select multiple dates; hold Shift for a range or drag to marquee-select.',
    '单次选择范围不能超过 10 年': 'A single selection cannot exceed 10 years',
    '也可以直接点击右侧月历中的日期进行标记。': 'You can also click dates in the month calendar to mark them.',
    '保存为新版本': 'Save as new version',
    '保存后，任务可在“业务日历”字段中绑定此版本': 'After saving, jobs can bind this calendar version.',
    '个节假日': 'holidays',
    '个调休工作日': 'make-up workdays',
    '当前画笔：': 'Current brush: ',
    '加载时区': 'Load time zones',
    '执行器': 'Executors',
    '执行记录': 'Executions',
    '节点与集群': 'Nodes & cluster',
    '插件': 'Plugins',
    ...
    '新建账号': 'New account',
    '编辑任务': 'Edit job',
    '还没有业务日历': 'No business calendars yet',
    '创建一套日历后，在任务编辑中绑定它。': 'Create a calendar and bind it when editing a job.',
    '按任务时区判断工作日、节假日和调休。': 'Working days, holidays, and make-up workdays are evaluated in the job time zone.',
    '名称': 'Name',
    '工作日': 'Workday',
    '每周工作日': 'Weekly workdays',
    '节假日': 'Holidays',
    '节假日 / 调休': 'Holidays / make-up workdays',
    '调休': 'Make-up workdays',
    '新建业务日历': 'New business calendar',
    '编辑业务日历': 'Edit business calendar',
    '规则按日期例外 → 每周模板的优先级执行': 'Rules run by priority: date exceptions → weekly template',
    '日历 ID': 'Calendar ID',
    '调休工作日': 'Make-up workdays',
    '保存日历': 'Save calendar',
    '不使用业务日历': 'No business calendar',
    '命中节假日时跳过；黑名单策略在任务配置中单独处理。': 'Holiday dates are skipped; blackout policy is configured separately on the job.',
    '编辑账号': 'Edit account',
    '关闭': 'Close',
    '取消': 'Cancel',
    '返回': 'Back',
