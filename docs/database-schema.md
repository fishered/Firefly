...
`firefly_executor_idempotency` 不属于调度中心 schema。它应创建在业务服务自己的数据库中，由 Executor JDBC 幂等 Store 管理业务消费 claim；对应的三种方言脚本位于 `clients/executor-netty` 模块。
...
- `firefly_executor_instance_location` 保存 Executor 实例所在 Gateway、连接会话 fencing、内部转发地址和短租约。注册、断连立即写入，心跳只续短租约；普通业务执行状态仍由 execution/outbox 表承载。
- `firefly_audit_log` 持久化 Admin 变更审计，记录操作者、角色、资源、结果、变更前后内容和时间。
- `firefly_job_history` 保存任务创建、启停和删除历史，供 `GET /api/jobs/{jobId}/history` 查询。
