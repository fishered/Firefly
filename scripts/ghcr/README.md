...
- The release commit has an annotated Git tag such as `v1.1.4`.
...
Run this first. It builds both images for `linux/amd64`, assigns the `1.1.4`
and `1.1` tags, and verifies the OCI version label. It does not upload images.

```powershell
.\scripts\ghcr\publish-ghcr.ps1 -Version 1.1.4
```
...
```powershell
.\scripts\ghcr\publish-ghcr.ps1 -Version 1.1.4 -Owner another-owner -Push
```
...
```powershell
.\scripts\ghcr\publish-ghcr.ps1 -Version 1.1.4 -NoPull
```
...
docker pull ghcr.io/fishered/firefly:1.1.4
docker pull ghcr.io/fishered/firefly-admin:1.1.4
```
