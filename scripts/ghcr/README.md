...
- The release commit has an annotated Git tag such as `v1.0.1`.
...
Run this first. It builds both images for `linux/amd64`, assigns the `1.0.1`
and `1.0` tags, and verifies the OCI version label. It does not upload images.

```powershell
.\scripts\ghcr\publish-ghcr.ps1 -Version 1.0.1
```
...
```powershell
.\scripts\ghcr\publish-ghcr.ps1 -Version 1.0.1 -Owner another-owner -Push
```
...
```powershell
.\scripts\ghcr\publish-ghcr.ps1 -Version 1.0.1 -NoPull
```
