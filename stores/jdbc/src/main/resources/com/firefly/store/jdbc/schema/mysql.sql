...
create table if not exists firefly_executor_instance_location (
    ...
    index idx_firefly_executor_location_lease (executor_name, status, lease_until, gateway_node_id)
);
...
create table if not exists firefly_audit_log (
    ...
    index idx_firefly_audit_time (occurred_at, audit_id)
);
...
create table if not exists firefly_job_history (
    ...
    after_payload longtext not null,
    occurred_at timestamp(6) not null,
    index idx_firefly_job_history_job (job_id, occurred_at, history_id)
);
